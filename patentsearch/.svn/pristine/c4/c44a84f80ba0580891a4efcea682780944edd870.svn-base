...
public class DaoSupport<T> implements DAO<T> {
	...
	public QueryResult<T> getScrollData(int firstindex, int maxresult,
			String wherejpql, Object[] params,
			LinkedHashMap<String, String> orderby,String zhaiYao,String zhuQuanLi) {
		QueryResult<T> qr = new QueryResult<T>();
 		Query query = em.createQuery("select o from " + getEntityName() + " o " +(wherejpql==null || "".equals(wherejpql)?"":"where "+wherejpql)+ getOrderByJpql(orderby));
		setQueryParam(query, params);
		if(firstindex != -1 && maxresult != -1){
	...
	public List<String> getAppnoListById(String wherejpql) {
		QueryResult qr = new QueryResult();
 		Query query = em.createQuery("select appno from " + getEntityName() + " o " +(wherejpql==null || "".equals(wherejpql)?"":"where "+wherejpql));
		
		qr.setResultlist(query.getResultList());
		return (List) qr.getResultlist();
	}
}
